...
const AddStudentScreen = () => {
  ...
  const fetchStudents = async () => {
    try {
      const response = await fetch(
        `${baseURL}/Student/getAllStudent?departmentid=${departmentId}`,
      );
  ...
  const handleDeleteStudent = async studentId => {
    try {
      const response = await fetch(
        `${baseURL}/user/deleteUser?studentId=${studentId}`,
        {
          method: 'DELETE',
          headers: {
    ...
    <View style={styles.itemContainer}>
      ...
      <View style={styles.iconContainer}>
        <Icon
          ...
          style={styles.icon}
          onPress={() =>
            navigation.navigate('EditStudentScreen', {student: item})
          }
        />
        <Icon
          name="trash"
          style={styles.icon}
          onPress={() => confirmDelete(item.studentId)}
        />
      </View>
    </View>
  );
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item?.studentId?.toString() || index.toString()
          }
        />
      )}
      <TouchableOpacity
        style={styles.addButton}
        onPress={() => navigation.navigate('AddSingleStudent')}>
        <Text style={styles.addButtonLabel}>Add Single Student</Text>
      </TouchableOpacity>
      <TouchableOpacity
        style={styles.addButton}
